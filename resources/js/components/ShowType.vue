        ...
        <div>
          ...
          <div v-if="showPayday">
            <div v-for="(item, index) in payDays" class="form-group pt-3 row justify-content-center">
              <label :for="(payDays[index].id == null ? 'due-days-new[]' : 'due-days[]')" class="col-form-label col-lg-5 offset-lg-1">{{ index + 1 }}º dia de pagamento: </label>

              <div class="pl-4 col-lg-4">
                <input type="number" class="form-control" :name="(payDays[index].id == null ? 'due-days-new[]' : 'due-days[]')" required autofocus v-model="payDays[index].due_day">
              </div>
